...
def handle_request():
    logger.debug("Get Hard Question Handle Request")
    cur = global_db_con.cursor()
    used_questions = request.form.getlist('used_hard_questions')
    numRows = 16
    random_question = -1
    checkedDupe = False
    ...
    cur.execute(f"select * from hard_questions where id = '{random_question}';")
    ...
    print("Successfully got hard question.")
