...
def handle_request():
    logger.debug("Get Medium Question Handle Request")
    cur = global_db_con.cursor()
    used_questions = request.form.getlist('used_medium_questions')
    numRows = 16
    random_question = -1
    checkedDupe = False
    ...
    cur.execute(f"select * from medium_questions where id = '{random_question}';")
    ...
    print("Successfully got medium question.")
